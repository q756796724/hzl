...
dailiflag = storage.get("dailiflag", true);
...
ui.layout(
    <vertical padding="16">
        ...
        <checkbox text="daili" id="dailiflag" checked="{{dailiflag}}" textSize="18sp" />\
...
ui.ok.click(function () {
    ...
    <com.stardust.autojs.core.console.ConsoleView    id="consoleLog"   />\
        ...
        function getjieshouCount() {
            let temp = null;
            try {
                temp = http.get("116.205.139.36:8081/fanqie/getjieshouCount");
        ...
        function addjieshouCount() {
            let temp = null;
            try {
                temp = http.get("116.205.139.36:8081/fanqie/addjieshouCount");
                if (temp && temp.statusCode == 200) {
                    temp = temp.body.string();
                    let rep = JSON.parse(temp);
                    let repState = rep["state"];
        ...
        function reducejieshouCount() {
            let temp = null;
            try {
                temp = http.get("116.205.139.36:8081/fanqie/reducejieshouCount");
                if (temp && temp.statusCode == 200) {
                    temp = temp.body.string();
                    let rep = JSON.parse(temp);
                    let repState = rep["state"];
        ...
        function getConfig() {
            let temp = null;
            try {
                temp = http.get("116.205.139.36:8081/fanqie/getConfig");
                if (temp && temp.statusCode == 200) {
                    temp = temp.body.string();
                    let rep = JSON.parse(temp);
                    let repState = rep["state"];
        ...
        function setConfig(lastTalkName, lastLinkTitle) {
            ...
            try {
                temp = http.postJson("116.205.139.36:8081/fanqie/setConfig", {
        ...
        function gotollb() {
            ...
            if (className("android.widget.TextView").textContains("lanlitao").findOne(5000) != null) {
                if (calcDateDayDiff(formatDate(new Date(), "yyyy-MM-dd"), xianzhidate) < 6&&havejieshouren(2) == false) {
                    return false;
                }
                阅读 = className("android.widget.TextView").textContains("lanlitao").findOnce().bounds();
                clickx(阅读.right, 阅读.bottom);
            ...
            let cBtn = packageName("com.tencent.mm").className("android.widget.TextView").text("专属链接").findOne(8000);
            if (cBtn != null) {
                log("2")
        ...
        function checkxianzhi_old() {
            ...
            if (gotollb()) {
                ...
                if (cBtn != null && cBtn.text() != undefined && cBtn.text() != "") {
                    阅读到底();
                    sleep(8000);
                    if (packageName("com.tencent.mm").className("android.view.View").textContains("文章出错了").findOne(5000) != null) {
                        log("限制")
                    ...
                    if (gotollb()) {
                        ...
                        if (cBtn != null && cBtn.text() != undefined && cBtn.text() != "") {
                            ...
                            if (!fanqieflag && checkxianzhiFlag == false && calcDateDayDiff(formatDate(new Date(), "yyyy-MM-dd"), xianzhidate) > 5) {
                                jieshouwenzhang();
                            }
                ...
                if (cBtn != null && cBtn.text() != undefined && cBtn.text() != "") {
                    if (yuedulanlibang()) {
                        if(dailiflag){
                            ...
                            if (配置["countllb"] < 20) {
                                checkxianzhiFlag = false;
                                log(new Date().toLocaleString() + "-----------小于20限制，次数："+配置["countllb"])
                                xianzhidate = formatDate(new Date(), "yyyy-MM-dd");
                                storage.put("xianzhidate", xianzhidate);
                                lunSleep()
                                lunSleep()
                    ...
                    if (dailiflag&&!fanqieflag && checkxianzhiFlag == false && calcDateDayDiff(formatDate(new Date(), "yyyy-MM-dd"), xianzhidate) > 5) {
                        jieshouwenzhang();
                    }
                } else if (packageName("com.tencent.mm").className("android.view.View").textContains("文章出错了").findOne(5000) != null) {
                    log("文章出错了限制")
            ...
            if(dailiflag){
                for (let i = 0; i < 5; i++) {
                    if (gotollb()) {
                        if (yuedulanlibang()) {
                            if (配置["countllb"] < 5) {
                                if(calcDateDayDiff(formatDate(new Date(), "yyyy-MM-dd"), xianzhidate) > 5){
       ...
       setInterval(结束未响应(), 6000);
        ...
        function onMainPage() {
            ...
            if (页面异常处理()) {
                ...
                if (yichang1count > 5) {
                    ...
                    if (checkxianzhiFlag == false && calcDateDayDiff(formatDate(new Date(), "yyyy-MM-dd"), xianzhidate) > 5) {
                        llbLunRead()
                        if(checkxianzhiFlag){
                            lunSleep()
                            return
                ...
                for (var i = 0; i < 5; i++) {
                    ...
                    if (className("android.view.View").textMatches(/(.*ZhaoLin|.*小青|.*miu|.*平和|.*韩玥|.*云雨)/).findOne(3000) != null) {
                        ...
                        if (yuedu()) {
                            ...
                            保存配置(settingPath, 配置);
                            //判断是否接收文章
                            /*if (jieshouwenzhangflag && lunCount == 2 && calcDateDayDiff(formatDate(new Date(), "yyyy-MM-dd"), xianzhidate) > 5) {//昨天限制今天可以的话第一轮不接收，昨天没限制第一轮接收
                                jieshouwenzhang();
        ...
        function yuedulanlibang() {
            if(dailiflag){
                ...
                for (let i = 0; i < 5; i++) {
                        ...
                        if (packageName("com.tencent.mm").className("android.view.View").textContains("文章出错了").findOne(10000) != null ) {
                            ...
                            if (cBtn != null && cBtn.text() != undefined && cBtn.text() != "") {
                                ...
                                if (count == 1 || count == 2|| count == 7||count == 8|| count == 11|| count == 13|| count == 14  ){
                                    if (checkxianzhiFlag == false && calcDateDayDiff(formatDate(new Date(), "yyyy-MM-dd"), xianzhidate) < 6) {
                    ...
                    if (count == 6 ||count == 10||count == 12 || count == 20) {
                        连接wifi(zwifi, 5000);
                        app.launch(PKG_NAME);
                        if (checkxianzhiFlag == false && calcDateDayDiff(formatDate(new Date(), "yyyy-MM-dd"), xianzhidate) < 6 && havejieshouren(1) == false) {
                            return true;
                        }
                    } else if (count % 5 == 0) {
                        if (联网验证(dlwifi) != true) {
        ...
        function yuedu() {
            ...
            for (; ;) {
                ...
                //判断是否需要互助
                if (count == wifiCount && calcDateDayDiff(formatDate(new Date(), "yyyy-MM-dd"), xianzhidate) < 6) {
        ...
        dailiflag = ui.dailiflag.isChecked();
        log("tx:" + auto_tx);
        ...
        storage.put("dailiflag", ui.dailiflag.isChecked());
        ...
        for (; ;) {
            ...
            sleep(5000);
            refreshStateInfo();
            if (topPackage == PKG_NAME && calcDateDayDiff(formatDate(new Date(), "yyyy-MM-dd"), xianzhidate) < 6) {
